...
class User:
    ...
    def getProfile(self):
        ...
        return { 'uid' : self.uid, 'updated' : datetime.datetime.utcnow(), 'datas' : {}}
...
class FriendFactory:
    ...
    def getFriends(self, uid, start = 0, limit = 10):
        try:
            Db().get('users').ensure_index([('friends.uid', storage.ASCENDING)], { 'background' : True })
            friends = Db().get('users').find_one({'uid' : uid}, {'friends.uid' : 1})
            ...
            data = Db().get('users').find({'uid' : { "$in" : list}, 'activate' : 1, 'friends.uid' : uid, 'activate' : 1 }, {'friends' : { '$slice' : 0 }  })
            total = data.count()
    ...
    def getMyRequests(self, uid, start = 0, limit = 10):
        try:
            ...
            for friend in friends['friends']:
                list.append(friend['uid'])
            data = Db().get('users').find({'uid' : { "$in" : list}, 'activate' : 1, 'friends.uid' : { '$nin' :  [uid] } , 'activate' : 1}, {'friends' : { '$slice' : 0 }  })
            total = data.count()
            data = data.skip(start).limit(limit)
    ...
    def request(self, me, uid):
        try:
            checkMe = Db().get('users').find({ 'uid' : me, 'friends.uid' : uid}).count()
            
            if checkMe > 0 :
                return False
            
            checkFriend = Db().get('users').find({ 'uid' : uid, 'activate' : 1, 'friends.uid' : me}).count()
            
            if checkFriend == 0:
                Db().get('users').update({ 'uid' : me}, { '$push' : {  'friends' : { 'uid' : uid, 'date' : datetime.datetime.utcnow()}}})
    ...
    def approve(self, me, uid):
        try:
            ...
            checkFriend = Db().get('users').find({ 'uid' : uid, 'friends.uid' : me}).count()
    ...
    def remove (self, me, uid):
        try:
            ...
            Db().get('users').update({ 'uid' : uid}, { '$inc' : { 'friends_count' : -1}, '$pull' : { 'friends' : { 'uid' : me }}}) #remove from his friend
    ...
    def deny (self, me, uid):
        try:
            checkMe = Db().get('users').find({ 'uid' : uid, 'friends.uid' : me}).count()
            
            if not checkMe :
                return False
            
            Db().get('users').update({ 'uid' : uid}, { '$pull' : { 'friends' : { 'uid' : me }}}) #remove from his friend
    ...
    def cancel (self, me, uid):
        try:
            ...
            Db().get('users').update({ 'uid' : me}, { '$pull' : { 'friends' : { 'uid' : uid }}}) #remove from my requests
...
class UserFactory:
    ...
    def get (self, search):
        try:
            ...
            data = storage.Db().get('users').find_one(search, {'friends' : { '$slice' : 0 }  })
    ...
    def getAllUsers(self):
        data = Db().get('users').find({'activate' : 1, 'seq' : { '$gt' : 0} }, {'friends' : { '$slice' : 0 }})
        result = []
        for friend in data:
            result.append(User(**friend))
    ...
    def new(self,  obj):
        ...
        Db().get('users').ensure_index([('uid', storage.DESCENDING)], { 'unique' : True, 'background' : False,  'dropDups' : True })
        Db().get('users').ensure_index([('email', storage.DESCENDING)], { 'unique' : True, 'background' : False,  'dropDups' : True })
