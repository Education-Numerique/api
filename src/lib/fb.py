...
class Auth:
    ...
    class __impl():
        ...
        def check(self, fbid, token):
            ...
            try:
                resp = Auth.SESSION.get( self.host + uri, verify = True, timeout = 5 )
        ...
        def getPicture(self, fbid):
            try:
                resp = Auth.SESSION.get(self.host + '/%s/picture?type=large' % fbid, verify=True, timeout = 5)
            except Exception as e:
                raise PictureError('picture fetch error : %s' % e)

            if resp.status_code != 200:
                raise PictureError('picture fetch error : status code' % resp.status)

            return resp.content

        def getSalt(self,token):
            return sha1((token + Config().get('social_secret')).encode('utf-8')).hexdigest()
