from lib import app
import time, json, re, datetime, hashlib
...
def success (msg, code):
    ...
    if not isinstance(msg, str):
        ...
        #bad bad bad perf
        dthandler = lambda obj: "%sZ" % obj.isoformat() if isinstance(obj, datetime.datetime) else None
...
def cacheManager(max = 3600 * 24 * 2, age = 0, handleEtags = False):
    ...
    resp.headers['Expires'] = time.strftime("%a, %d %b %Y %H:%M:%S GMT", time.gmtime(expire))
...
def noCache():
    ...
    resp.headers['Last-Modified'] = time.strftime("%a, %d %b %Y %H:%M:%S GMT", time.gmtime(future))
...
def __getAge ():
    ...
    #localtime
    lastRefresh = datetime.datetime.now() - datetime.timedelta(days=days, seconds=int(delta.seconds))
    #get utc timestamp
