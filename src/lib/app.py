    ...
    class __impl ():
        ...
        def __call__(self, environ, start_response):
            ...
            if 'application/json' in environ.get('CONTENT_TYPE'):
                environ['CONTENT_TYPE'] = environ['CONTENT_TYPE'].replace('application/json','application/x-www-form-urlencoded')
                isJson = True
            ...
            if self._req.method == 'POST' and isJson:
                ...
                try:
                    self._json = loads(self._req.body.decode('utf-8'), encoding=self._req.charset)
                except Exception as e:
                    self._json = None
                 
                if self._json != None and isinstance(self._json, dict):
                    post = []
                    ...
                    self._req.body = urllib.parse.urlencode(post).encode('utf-8')

                elif self._json != None and isinstance(self._json, list):
                    post = []
        ...
        def checkToken(self):
            ...
            challenge = sha1((str(info[0]) + Config().get('token_secret')).encode('utf-8')).hexdigest()
        ...
        def addRoutingFromConfig(self, domain, routing):
            ...
            for entry in routing:
                ...
                entries += (url, ServiceLoader().get(params['service'], params['module']), vars, params)
        ...
        def getResponse (self, flush = False):
            if (self._resp == None) :
                self._resp = Response(content_type='application/json', charset='UTF-8')
