...
class Db:
    ...
    class __impl ():
        ...
        def connect(self):
            try:
                connection = pymongo.Connection(self.__cfg.get('host'), self.__cfg.get('port'))
...
class MyCollection:
    ...
    def increment(self, namespace = None):
        key = self.__cnx.name if not namespace else '%s.%s' % (self.__cnx.name, namespace)
        resp = Db().get('seq').find_and_modify({'_id' : key}, {'$inc': {"seq":1}}, True, new=True)
        return resp['seq']
